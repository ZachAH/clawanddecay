...
export const handler = async (event, context) => {
  ...
  try {
    ...
    const FIREBASE_BUCKET_NAME = process.env.FIREBASE_STORAGE_BUCKET; // Get bucket name for URL construction
    ...
    const productsWithRewrittenImages = productsData.data.map(product => {
        ...
        product.images = (product.images || []).map(image => {
            ...
            // Ensure FIREBASE_BUCKET_NAME is correctly set in Netlify environment variables
